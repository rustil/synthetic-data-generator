...
class DCGAN_G_nonSeq(nn.Module):
    """ 
        generator component of WGAN
    """
    def __init__(self, ngf, nz):
        super(DCGAN_G_nonSeq, self).__init__()
        ...
        self.tconv1 = nn.ConvTranspose3d(ngf*8, ngf*4, kernel, 2, 1, bias=False)
        self.lnorm_1 = nn.LayerNorm([8, 8, 8])
        
        # state shape [ (ndf*4) x 8 x 8 ]

        self.tconv2 = nn.ConvTranspose3d(ngf*4, ngf*2, kernel, 2, 1, bias=False)
        self.lnorm_2 = nn.LayerNorm([16, 16, 16])
        
        # state shape [ (ndf*2) x 16 x 16 ]

        self.tconv3 = nn.ConvTranspose3d(ngf*2, ngf, kernel, 2, 1, bias=False)
        self.lnorm_3 = nn.LayerNorm([32, 32, 32])
  
        # state shape [ (ndf) x 32 x 32 ]

        self.tconv4 = nn.ConvTranspose3d(ngf, 1, 3, 1, 2, bias=False)
    ...
    def forward(self, noise, energy):
        ...
        x = F.relu(self.lnorm_1(self.tconv1(input)))
        x = F.relu(self.lnorm_2(self.tconv2(x)))
        x = F.relu(self.lnorm_3(self.tconv3(x)))
