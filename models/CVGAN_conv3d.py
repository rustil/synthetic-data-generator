...
class Generator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.conv1_1 = nn.ConvTranspose3d(1, ngf*4, karnel_G, 1, 0, bias=False)
        # state size [ ngf*4 x 4 x 4 ]

        # input noise shape [batch x nz x 1 x 1] going into convolutional
        self.conv1_100 = nn.ConvTranspose3d(nz, ngf*4, karnel_G, 1, 0, bias=False)
        # state size [ ngf*4 x 4 x 4 ]
    ...
    def forward(self, noise, energy):
        ...
        input = torch.cat((energy_trans, noise_trans), 1) #.reshape(len(energy), 256, 1, 4, 4)
